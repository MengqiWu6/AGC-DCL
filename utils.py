...
def preprocess_graph(adj, layer, norm='sym', renorm=True):
    ...
    if norm == 'sym':
        ...
        laplacian = ident - adj_normalized
    #laplacian1=laplacian.toarray()
    #a=np.linalg.eig(laplacian1)
    #print("该矩阵的特征值为：",a[0])
    #print("该矩阵的最大特征值为：",np.max(a[0]))
    # reg = [2 / 3] * (layer)
    reg = [1] * (layer)            #uat:5/7  cora citeseer amap:2/3 bat:10/13 eat:5/7
...
def clustering(feature, true_labels, cluster_num):
    ...
    predict_labels, dis, initial = kmeans(X=feature, num_clusters=cluster_num, distance="euclidean", device="cuda:0")
...
def pseudo_matrix(S):
    ...
    M = torch.abs(0 - S_norm) ** args.beta
